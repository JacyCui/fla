...
有了上一题的基础，这一题就好做多了，我们可以把上一题的图灵机当成是一个子过程 (sub-procedure)，先直接右移，然后将最右边的挪到最左边去就可以了。
...
类似的，不仅仅只是数 4 个符号，我们可以数任意个符号。相比于下推自动机只能数两个，多了一个栈，能力就发生了翻天覆地的变化，从下推自动机直接进化成了图灵机。
...
到目前为止，我们所说的图灵机都是 **确定性图灵机 (Deterministic Turing Machine, DTM)**，即图灵机在相同状态下读到相同输入的时候只会有一种移动情况。
...
首先，对 $n$ 执行查找操作，如果没有找到，则将 `n*v#` 插入在存储纸带的末尾。
...
使用第三个纸带，将 `v'` 之后的所有内容拷贝到这第三个纸带上面。不过在此之前，你需要将 `v'` 之前的 `*` 的位置标记下来。
